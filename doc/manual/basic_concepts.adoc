...
image::DataFlowDiagram.png[Data flow diagram,pdfwidth=60%]
...
The separation between user interface and actual simulator makes it very easy to use _MasterSim_ in a scripted environment, or for systematic variant analysis as described below in section <<Workflows>>.
...
See chapter <<development_help.adoc, Features assisting in FMU development and debugging>> for details about functionality that are particularly important for FMU developers and for debugging problems with the co-simulation.
...
master time:: time of master simulation, starts with 0; unit is not strictly defined (needs to be common agreement between FMUs, typically in seconds, with exception of using file reader slaves, see section <<project_reference.adoc#_csv_filereader_slaves,CSV FileReader Slaves>>).
...
(see `--working-dir` command line argument described in section <<command_line_solver.adoc#trueworkingoutput_directory, Working/output directory>>).
====
