...
=== Building MasterSim for FMU Debugging

*Problem*: _MasterSim_ uses the IBK library, and by default, when developing in Qt Creator, this library is dynamically linked. When now _MasterSim_ loads an FMU library at runtime, which _also_ depends on the IBK library, any duplicate symbols/functions are not imported, but instead the FMU library will use the routines already linked into _MasterSim_. 

This is not a problem if:

- the IBK library versions are exactly the same
- both, _MasterSim_ and the FMU are built the same (either debug or release mode)

In other cases this leads to crashes while debugging.

*Solution*: You need to link the IBK library statically into _MasterSim_, but also all other libraries that depend on the IBK library. For convenience, you can just modify the qmake pro configuration by opening the file `externals/IBK/projects/Qt/CONFIG.pri` and enable the line:
```
OPTIONS += FMU_Debugging
```

Now delete all already generated shared lib files from folder `externals/lib_x64` and `lib_x64`, rebuild all and you are ready to go.
