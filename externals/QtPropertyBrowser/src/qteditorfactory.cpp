...
// QtDoubleLineEditFactory

class QtDoubleLineEditFactoryPrivate : public EditorFactoryPrivate<QPW::ValidatingLineEdit>
{
	QtDoubleLineEditFactory *q_ptr;
	Q_DECLARE_PUBLIC(QtDoubleLineEditFactory)
public:
...
void QtDoubleLineEditFactoryPrivate::slotPropertyChanged(QtProperty *property, double value)
{
	QList<QPW::ValidatingLineEdit *> editors = m_createdEditors[property];
	QListIterator<QPW::ValidatingLineEdit *> itEditor(m_createdEditors[property]);
	while (itEditor.hasNext()) {
		QPW::ValidatingLineEdit *editor = itEditor.next();
		if (editor->value() != value) {
			editor->blockSignals(true);
			editor->setValue(value);
			editor->blockSignals(false);
...
void QtDoubleLineEditFactoryPrivate::slotRangeChanged(QtProperty *property,
			double min, double max, QString toolTip)
{
	if (!m_createdEditors.contains(property))
		return;

	QtDoubleEditPropertyManager *manager = q_ptr->propertyManager(property);
	if (!manager)
		return;

	QList<QPW::ValidatingLineEdit *> editors = m_createdEditors[property];
	QListIterator<QPW::ValidatingLineEdit *> itEditor(editors);
	while (itEditor.hasNext()) {
		QPW::ValidatingLineEdit *editor = itEditor.next();
		editor->blockSignals(true);
		editor->setup(min, max, toolTip);
		editor->setValue(manager->value(property));
		editor->blockSignals(false);
	}
}

void QtDoubleLineEditFactoryPrivate::slotSetValue(double value)
{
	QObject *object = q_ptr->sender();
	const QMap<QPW::ValidatingLineEdit *, QtProperty *>::ConstIterator itcend = m_editorToProperty.constEnd();
	for (QMap<QPW::ValidatingLineEdit *, QtProperty *>::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != itcend; ++itEditor) {
		if (itEditor.key() == object) {
			QtProperty *property = itEditor.value();
			QtDoubleEditPropertyManager *manager = q_ptr->propertyManager(property);
			if (!manager)
...
/*! \class QtDoubleLineEditFactory
	\internal
	\inmodule QtDesigner
	\since 5.7

	\brief The QtDoubleLineEditFactory class provides QPW::ValidatingLineEdit
	widgets for properties created by QtDoubleEditPropertyManager objects.

	\sa QtAbstractEditorFactory, QtDoubleEditPropertyManager
*/

/*!
	Creates a factory with the given \a parent.
*/
QtDoubleLineEditFactory::QtDoubleLineEditFactory(QObject *parent)
	: QtAbstractEditorFactory<QtDoubleEditPropertyManager>(parent), d_ptr(new QtDoubleLineEditFactoryPrivate())
{
	d_ptr->q_ptr = this;
...
/*!
	Destroys this factory, and all the widgets it has created.
*/
QtDoubleLineEditFactory::~QtDoubleLineEditFactory()
{
	qDeleteAll(d_ptr->m_editorToProperty.keys());
}
...
/*!
	...
	Reimplemented from the QtAbstractEditorFactory class.
*/
void QtDoubleLineEditFactory::connectPropertyManager(QtDoubleEditPropertyManager *manager)
{
	connect(manager, SIGNAL(valueChanged(QtProperty*,double)),
				this, SLOT(slotPropertyChanged(QtProperty*,double)));
	connect(manager, SIGNAL(rangeChanged(QtProperty*,double,double,QString)),
...
/*!
	...
	Reimplemented from the QtAbstractEditorFactory class.
*/
QWidget *QtDoubleLineEditFactory::createEditor(QtDoubleEditPropertyManager *manager,
		QtProperty *property, QWidget *parent)
{
	QPW::ValidatingLineEdit *editor = d_ptr->createEditor(property, parent);
	editor->setup(manager->minimum(property), manager->maximum(property), "");
	editor->setValue(manager->value(property));
//	editor->setKeyboardTracking(false);
...
/*!
	...
	Reimplemented from the QtAbstractEditorFactory class.
*/
void QtDoubleLineEditFactory::disconnectPropertyManager(QtDoubleEditPropertyManager *manager)
{
	disconnect(manager, SIGNAL(valueChanged(QtProperty*,double)),
				this, SLOT(slotPropertyChanged(QtProperty*,double)));
	disconnect(manager, SIGNAL(rangeChanged(QtProperty*,double,double,QString)),
				this, SLOT(slotRangeChanged(QtProperty*,double,double,Qstring)));
}

// QtCoordinateIndexEditFactory

class QtCoordinateIndexEditFactoryPrivate : public EditorFactoryPrivate<QPW::CoordinateIndexEdit>
{
	QtCoordinateIndexEditFactory *q_ptr;
	Q_DECLARE_PUBLIC(QtCoordinateIndexEditFactory)
public:

	void slotPropertyChanged(QtProperty *property, int value);
	void slotCoordinateVectorChanged(QtProperty *property, const QVector<double> & coordinates);
	void slotSpaceTypeChanged(QtProperty *property, bool spaceType);
	void slotTimeUnitBaseChanged(QtProperty *property, int baseUnit);
	void slotTimeUnitDisplayChanged(QtProperty *property, int displayUnit);
	void slotSetValue(int value);
	void slotSetCoordinateVector(const QVector<double> & coordinates);
};

void QtCoordinateIndexEditFactoryPrivate::slotPropertyChanged(QtProperty *property, int value)
{
	QList<QPW::CoordinateIndexEdit *> editors = m_createdEditors[property];
	QListIterator<QPW::CoordinateIndexEdit *> itEditor(m_createdEditors[property]);
	while (itEditor.hasNext()) {
		QPW::CoordinateIndexEdit *editor = itEditor.next();
		if (editor->index() != value) {
			editor->blockSignals(true);
			editor->setIndex(value);
			editor->blockSignals(false);
		}
	}
}

void QtCoordinateIndexEditFactoryPrivate::slotCoordinateVectorChanged(QtProperty *property, const QVector<double> & coordinates)
{
	QList<QPW::CoordinateIndexEdit *> editors = m_createdEditors[property];
	QListIterator<QPW::CoordinateIndexEdit *> itEditor(m_createdEditors[property]);
	while (itEditor.hasNext()) {
		QPW::CoordinateIndexEdit *editor = itEditor.next();
		editor->blockSignals(true);
		editor->set(coordinates);
		editor->blockSignals(false);
	}
}

void QtCoordinateIndexEditFactoryPrivate::slotSpaceTypeChanged(QtProperty *property, bool spaceType)
{
	QList<QPW::CoordinateIndexEdit *> editors = m_createdEditors[property];
	QListIterator<QPW::CoordinateIndexEdit *> itEditor(m_createdEditors[property]);
	while (itEditor.hasNext()) {
		QPW::CoordinateIndexEdit *editor = itEditor.next();
		editor->blockSignals(true);
		editor->setCoordinateType(spaceType);
		editor->blockSignals(false);
	}
}

void QtCoordinateIndexEditFactoryPrivate::slotTimeUnitBaseChanged(QtProperty *property, int baseUnit)
{
	QList<QPW::CoordinateIndexEdit *> editors = m_createdEditors[property];
	QListIterator<QPW::CoordinateIndexEdit *> itEditor(m_createdEditors[property]);
	while (itEditor.hasNext()) {
		QPW::CoordinateIndexEdit *editor = itEditor.next();
		editor->blockSignals(true);
		editor->setTimeUnits(static_cast<QPW::CoordinateIndexEdit::TimeUnit>(baseUnit), QPW::CoordinateIndexEdit::TU_Unknown);
		editor->blockSignals(false);
	}
}

void QtCoordinateIndexEditFactoryPrivate::slotTimeUnitDisplayChanged(QtProperty *property, int displayUnit)
{
	QList<QPW::CoordinateIndexEdit *> editors = m_createdEditors[property];
	QListIterator<QPW::CoordinateIndexEdit *> itEditor(m_createdEditors[property]);
	while (itEditor.hasNext()) {
		QPW::CoordinateIndexEdit *editor = itEditor.next();
		editor->blockSignals(true);
		editor->setTimeUnits(QPW::CoordinateIndexEdit::TU_Unknown, static_cast<QPW::CoordinateIndexEdit::TimeUnit>(displayUnit));
		editor->blockSignals(false);
	}
}


void QtCoordinateIndexEditFactoryPrivate::slotSetValue(int value)
{
	QObject *object = q_ptr->sender();
	const QMap<QPW::CoordinateIndexEdit *, QtProperty *>::ConstIterator itcend = m_editorToProperty.constEnd();
	for (QMap<QPW::CoordinateIndexEdit *, QtProperty *>::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != itcend; ++itEditor) {
		if (itEditor.key() == object) {
			QtProperty *property = itEditor.value();
			QtCoordinateIndexPropertyManager *manager = q_ptr->propertyManager(property);
			if (!manager)
				return;
			manager->setValue(property, value);
			return;
...
void QtCoordinateIndexEditFactoryPrivate::slotSetCoordinateVector(const QVector<double> & coordinates) {
	QObject *object = q_ptr->sender();
	const QMap<QPW::CoordinateIndexEdit *, QtProperty *>::ConstIterator itcend = m_editorToProperty.constEnd();
	for (QMap<QPW::CoordinateIndexEdit *, QtProperty *>::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != itcend; ++itEditor) {
		if (itEditor.key() == object) {
			...
			if (!manager)
				return;
			manager->setCoordinateVector(property, coordinates);
			return;
		}
	}
}

/*! \class QtCoordinateIndexEditFactory
	\internal
	\inmodule QtDesigner

	\brief The QtCoordinateIndexEditFactory class provides QPW::ValidatingLineEdit
	widgets for properties created by QtDoubleEditPropertyManager objects.

	\sa QtAbstractEditorFactory, QtCoordinateIndexPropertyManager
*/

/*!
	Creates a factory with the given \a parent.
*/
QtCoordinateIndexEditFactory::QtCoordinateIndexEditFactory(QObject *parent)
	: QtAbstractEditorFactory<QtCoordinateIndexPropertyManager>(parent), d_ptr(new QtCoordinateIndexEditFactoryPrivate())
{
	d_ptr->q_ptr = this;
...
/*!
	Destroys this factory, and all the widgets it has created.
*/
QtCoordinateIndexEditFactory::~QtCoordinateIndexEditFactory()
{
	qDeleteAll(d_ptr->m_editorToProperty.keys());
}
...
/*!
	...
	Reimplemented from the QtAbstractEditorFactory class.
*/
void QtCoordinateIndexEditFactory::connectPropertyManager(QtCoordinateIndexPropertyManager *manager)
{
	connect(manager, SIGNAL(valueChanged(QtProperty*,int)),
				this, SLOT(slotPropertyChanged(QtProperty*,int)));
	connect(manager, SIGNAL(coordinateVectorChanged(QtProperty *, const QVector<double> & )),
			this, SLOT(slotCoordinateVectorChanged(QtProperty *, const QVector<double> &)));
	connect(manager, SIGNAL(coordinateSpaceTypeChanged(QtProperty *, bool)),
			this, SLOT(slotSpaceTypeChanged(QtProperty *, bool)));
	connect(manager, SIGNAL(coordinateTimeUnitBaseChanged(QtProperty *, int)),
...
/*!
	...
	Reimplemented from the QtAbstractEditorFactory class.
*/
QWidget *QtCoordinateIndexEditFactory::createEditor(QtCoordinateIndexPropertyManager *manager,
		QtProperty *property, QWidget *parent)
{
	QPW::CoordinateIndexEdit *editor = d_ptr->createEditor(property, parent);
	editor->set(manager->coordinateVector(property));
	editor->setCoordinateType(manager->spaceType(property));
	editor->setTimeUnits(static_cast<QPW::CoordinateIndexEdit::TimeUnit>(manager->timeUnitBase(property)),
						 static_cast<QPW::CoordinateIndexEdit::TimeUnit>(manager->timeUnitDisplay(property)));
	editor->setIndex(manager->value(property));
...
/*!
	...
	Reimplemented from the QtAbstractEditorFactory class.
*/
void QtCoordinateIndexEditFactory::disconnectPropertyManager(QtCoordinateIndexPropertyManager *manager)
{
	disconnect(manager, SIGNAL(valueChanged(QtProperty*,int)),
				this, SLOT(slotPropertyChanged(QtProperty*,int)));
	disconnect(manager, SIGNAL(coordinateVectorChanged(QtProperty *, const QVector<double> & )),
			this, SLOT(slotCoordinateVectorChanged(QtProperty *, const QVector<double> &)));
	disconnect(manager, SIGNAL(coordinateSpaceTypeChanged(QtProperty *, bool)),
			this, SLOT(slotSpaceTypeChanged(QtProperty *, bool)));
	disconnect(manager, SIGNAL(coordinateTimeUnitBaseChanged(QtProperty *, int)),
			this, SLOT(slotTimeUnitBaseChanged(QtProperty *, int)));
	disconnect(manager, SIGNAL(coordinateTimeUnitBaseChanged(QtProperty *, int)),
			this, SLOT(slotTimeUnitDisplayChanged(QtProperty *, int)));
}
...
#if QT_VERSION >= 0x050000
  #define KEY_SEQUENCE_EDIT_TYPE QKeySequenceEdit
#else
  #define KEY_SEQUENCE_EDIT_TYPE QtKeySequenceEdit
#endif

class QtKeySequenceEditorFactoryPrivate : public EditorFactoryPrivate<KEY_SEQUENCE_EDIT_TYPE>
{
			...
			const QKeySequence &value)
{
	...
	QListIterator<KEY_SEQUENCE_EDIT_TYPE *> itEditor(m_createdEditors[property]);
	while (itEditor.hasNext()) {
		KEY_SEQUENCE_EDIT_TYPE *editor = itEditor.next();
		editor->blockSignals(true);
...
{
	QObject *object = q_ptr->sender();
	const  QMap<KEY_SEQUENCE_EDIT_TYPE *, QtProperty *>::ConstIterator ecend = m_editorToProperty.constEnd();
	for (QMap<KEY_SEQUENCE_EDIT_TYPE *, QtProperty *>::ConstIterator itEditor =  m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor)
		if (itEditor.key() == object) {
			QtProperty *property = itEditor.value();
...
QWidget *QtKeySequenceEditorFactory::createEditor(QtKeySequencePropertyManager *manager,
		QtProperty *property, QWidget *parent)
{
	KEY_SEQUENCE_EDIT_TYPE *editor = d_ptr->createEditor(property, parent);
...
// QtDateTimeFormatEditWidget

class QtDateTimeFormatEditWidget : public QWidget {
	Q_OBJECT

public:
	QtDateTimeFormatEditWidget(QWidget *parent);
...
QtDateTimeFormatEditWidget::QtDateTimeFormatEditWidget(QWidget *parent) :
	QWidget(parent),
	m_label(new QLabel)
{
	QHBoxLayout *lt = new QHBoxLayout(this);
	setupTreeViewEditorMargin(lt);
	lt->addWidget(m_label);
	m_label->setText(tr("Date/time format strings"));
}

void QtDateTimeFormatEditWidget::setValue(const QStringList &sl) {
	if (m_formats != sl) {
		m_formats = sl;
		m_label->setText(tr("Date/time format strings"));
	}
...
// QtDateTimeFormatEditorFactoryPrivate

class QtDateTimeFormatEditorFactoryPrivate : public EditorFactoryPrivate<QtDateTimeFormatEditWidget>
{
	QtDateTimeFormatEditorFactory *q_ptr;
	Q_DECLARE_PUBLIC(QtDateTimeFormatEditorFactory)
public:

	void slotPropertyChanged(QtProperty *property, const QStringList &value);
	void slotSetValue(const QStringList &value);
};

void QtDateTimeFormatEditorFactoryPrivate::slotPropertyChanged(QtProperty *property,
				const QStringList &value)
{
	const PropertyToEditorListMap::iterator it = m_createdEditors.find(property);
	if (it == m_createdEditors.end())
		return;
	QListIterator<QtDateTimeFormatEditWidget *> itEditor(it.value());

	while (itEditor.hasNext())
		itEditor.next()->setValue(value);
}

void QtDateTimeFormatEditorFactoryPrivate::slotSetValue(const QStringList &value)
{
	QObject *object = q_ptr->sender();
	const EditorToPropertyMap::ConstIterator ecend = m_editorToProperty.constEnd();
	for (EditorToPropertyMap::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor)
		if (itEditor.key() == object) {
			QtProperty *property = itEditor.value();
			QtDateTimeFormatPropertyManager *manager = q_ptr->propertyManager(property);
			if (!manager)
				return;
			manager->setValue(property, value);
			return;
...
/*!
	\class QtDateTimeFormatEditorFactory
	\internal

	\brief The QtDateTimeFormatEditorFactory class provides editing format strings for
	properties created by QtDateTimeFormatPropertyManager objects.

	\sa QtAbstractEditorFactory, QtDateTimeFormatPropertyManager
*/

/*!
	Creates a factory with the given \a parent.
*/
QtDateTimeFormatEditorFactory::QtDateTimeFormatEditorFactory(QObject *parent) :
	QtAbstractEditorFactory<QtDateTimeFormatPropertyManager>(parent),
	d_ptr(new QtDateTimeFormatEditorFactoryPrivate())
{
	d_ptr->q_ptr = this;
}
...
/*!
	Destroys this factory, and all the widgets it has created.
*/
QtDateTimeFormatEditorFactory::~QtDateTimeFormatEditorFactory()
{
	qDeleteAll(d_ptr->m_editorToProperty.keys());
}
...
/*!
	...
	Reimplemented from the QtAbstractEditorFactory class.
*/
void QtDateTimeFormatEditorFactory::connectPropertyManager(QtDateTimeFormatPropertyManager *manager)
{
	connect(manager, SIGNAL(valueChanged(QtProperty*,QStringList)),
			this, SLOT(slotPropertyChanged(QtProperty*,QStringList)));
}
...
/*!
	...
	Reimplemented from the QtAbstractEditorFactory class.
*/
QWidget *QtDateTimeFormatEditorFactory::createEditor(QtDateTimeFormatPropertyManager *manager,
		QtProperty *property, QWidget *parent)
{
	QtDateTimeFormatEditWidget *editor = d_ptr->createEditor(property, parent);
	editor->setValue(manager->value(property));
	connect(editor, SIGNAL(valueChanged(QStringList)), this, SLOT(slotSetValue(QStringList)));
	connect(editor, SIGNAL(destroyed(QObject*)), this, SLOT(slotEditorDestroyed(QObject*)));
	return editor;
}
...
/*!
	...
	Reimplemented from the QtAbstractEditorFactory class.
*/
void QtDateTimeFormatEditorFactory::disconnectPropertyManager(QtDateTimeFormatPropertyManager *manager)
{
	disconnect(manager, SIGNAL(valueChanged(QtProperty*,QStringList)), this, SLOT(slotPropertyChanged(QtProperty*,QStringList)));
}
