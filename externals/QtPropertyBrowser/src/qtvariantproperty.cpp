...
class QtDoubleEditPropertyType
{
};

class QtDateTimeFormatPropertyType
{
};

class QtCoordinateIndexType
{
};
...
Q_DECLARE_METATYPE(QtDoubleEditPropertyType)
Q_DECLARE_METATYPE(QtDateTimeFormatPropertyType)
Q_DECLARE_METATYPE(QtCoordinateIndexType)
...
/*!
	Returns the type id for a double edit property.

	Note that the property's value type can be retrieved using the
	valueType() function (which is QVariant::Invalid for the group
	property type, since it doesn't provide any value).
	It is used for editing double values by using a line edit.

	\sa propertyType(), valueType()
*/
int QtVariantPropertyManager::doubleEditTypeId()
{
	return qMetaTypeId<QtDoubleEditPropertyType>();
}

/*!
	Returns the type id for a DateTime format property.

	Note that the property's value type can be retrieved using the
	valueType() function (which is QVariant::Invalid for the group
	property type, since it doesn't provide any value).
	It is used for editing date time format strings for time intervals.

	\sa propertyType(), valueType()
*/
int QtVariantPropertyManager::datTimeFormatTypeId()
{
	return qMetaTypeId<QtDateTimeFormatPropertyType>();
}

/*!
	Returns the type id for a coordinate index property.
	...
	\sa propertyType(), valueType()
*/
int QtVariantPropertyManager::coordinateIndexTypeId()
{
	return qMetaTypeId<QtCoordinateIndexType>();
}
...
/*!
	...
	\l{QtVariantPropertyManager::doubleEditTypeId()}{doubleEditTypeId()}.
	\l{QtVariantPropertyManager::datTimeFormatTypeId()}{datTimeFormatTypeId()}.
	\l{QtVariantPropertyManager::coordinateIndexTypeId()}{coordinateIndexTypeId()}.
...
public:
	...
	void slotRangeChanged(QtProperty *property, int min, int max, QString toolTip);
	void slotSingleStepChanged(QtProperty *property, int step);
	void slotValueChanged(QtProperty *property, double val);
	void slotRangeChanged(QtProperty *property, double min, double max);
	void slotRangeChanged(QtProperty *property, double min, double max, QString toolTip);
	...
	void slotVectorChanged(QtProperty *property, const QVector<double> &values);
	void slotSpaceTypeChanged(QtProperty *property, bool space);
	void slotTimeUnitBaseChanged(QtProperty *property, int timeUnit);
	void slotTimeUnitDisplayChanged(QtProperty *property, int timeUnit);
	...
	const QString m_toolTipAttribute;
	const QString m_coordinateVectorAttribute;
	const QString m_coordianteSpaceTypeAttribute;
	const QString m_coordianteTimeUnitBaseAttribute;
	const QString m_coordianteTimeUnitDisplayAttribute;
};
...
QtVariantPropertyManagerPrivate::QtVariantPropertyManagerPrivate() :
	...
	m_toolTipAttribute("tool tip"),
	m_coordinateVectorAttribute(QLatin1String("coordinateVector")),
	m_coordianteSpaceTypeAttribute(QLatin1String("coordinateSpaceType")),
	m_coordianteTimeUnitBaseAttribute(QLatin1String("coordinateTimeUnitBase")),
	m_coordianteTimeUnitDisplayAttribute(QLatin1String("coordinateTimeUnitDisplay"))
{
}
...
int QtVariantPropertyManagerPrivate::internalPropertyToType(QtProperty *property) const
{
	...
	if (qobject_cast<QtIntPropertyManager *>(internPropertyManager))
		type = QVariant::Int;
	if (qobject_cast<QtStringPropertyManager *>(internPropertyManager))
	...
	else if (qobject_cast<QtDoubleEditPropertyManager *>(internPropertyManager))
		type = QtVariantPropertyManager::doubleEditTypeId();
	else if (qobject_cast<QtDateTimeFormatPropertyManager *>(internPropertyManager))
		type = QtVariantPropertyManager::datTimeFormatTypeId();
	else if (qobject_cast<QtCoordinateIndexPropertyManager *>(internPropertyManager))
		type = QtVariantPropertyManager::coordinateIndexTypeId();
	return type;
}
...
void QtVariantPropertyManagerPrivate::slotRangeChanged(QtProperty *property, int min, int max, QString toolTip)
{
	if (QtVariantProperty *varProp = m_internalToProperty.value(property, 0)) {
		emit q_ptr->attributeChanged(varProp, m_minimumAttribute, QVariant(min));
		emit q_ptr->attributeChanged(varProp, m_maximumAttribute, QVariant(max));
		emit q_ptr->attributeChanged(varProp, m_toolTipAttribute, QVariant(toolTip));
	}
}
...
void QtVariantPropertyManagerPrivate::slotRangeChanged(QtProperty *property, double min, double max, QString toolTip)
{
	if (QtVariantProperty *varProp = m_internalToProperty.value(property, 0)) {
		emit q_ptr->attributeChanged(varProp, m_minimumAttribute, QVariant(min));
		emit q_ptr->attributeChanged(varProp, m_maximumAttribute, QVariant(max));
...
void QtVariantPropertyManagerPrivate::slotVectorChanged(QtProperty *property, const QVector<double> & values)
{
	if (QtVariantProperty *varProp = m_internalToProperty.value(property, 0)) {
		QVariant v;
		v.fromValue(values);
		emit q_ptr->attributeChanged(varProp, m_coordinateVectorAttribute, v);
	}
}

void QtVariantPropertyManagerPrivate::slotSpaceTypeChanged(QtProperty *property, bool space)
{
	if (QtVariantProperty *varProp = m_internalToProperty.value(property, 0)) {
		emit q_ptr->attributeChanged(varProp, m_coordianteSpaceTypeAttribute, space);
	}
}

void QtVariantPropertyManagerPrivate::slotTimeUnitBaseChanged(QtProperty *property, int timeUnit)
{
	if (QtVariantProperty *varProp = m_internalToProperty.value(property, 0)) {
		emit q_ptr->attributeChanged(varProp, m_coordianteTimeUnitBaseAttribute, timeUnit);
	}
}

void QtVariantPropertyManagerPrivate::slotTimeUnitDisplayChanged(QtProperty *property, int timeUnit)
{
	if (QtVariantProperty *varProp = m_internalToProperty.value(property, 0)) {
		emit q_ptr->attributeChanged(varProp, m_coordianteTimeUnitDisplayAttribute, timeUnit);
	}
}
...
/*!
	...
	\row
		...
		\li doubleEditTypeId()
	\row
		\li double coordinate
		\li coordinateIndexTypeId()
	\row
		\li bool
		\li QVariant::Bool
	\row
	...
	: QtAbstractPropertyManager(parent), d_ptr(new QtVariantPropertyManagerPrivate)
{
	...
	// DoubleEditPropertyManager
	int doubleEditId = doubleEditTypeId();
	QtDoubleEditPropertyManager *doubleEditPropertyManager = new QtDoubleEditPropertyManager(this);
	d_ptr->m_typeToPropertyManager[doubleEditId] = doubleEditPropertyManager;
	d_ptr->m_typeToAttributeToAttributeType[doubleEditId][d_ptr->m_minimumAttribute] =
			QVariant::Double;
	d_ptr->m_typeToAttributeToAttributeType[doubleEditId][d_ptr->m_maximumAttribute] =
			QVariant::Double;
	d_ptr->m_typeToAttributeToAttributeType[doubleEditId][d_ptr->m_toolTipAttribute] =
			QVariant::String;
	d_ptr->m_typeToAttributeToAttributeType[doubleEditId][d_ptr->m_decimalsAttribute] =
			QVariant::Int;
	d_ptr->m_typeToValueType[doubleEditId] = QVariant::Double;
	connect(doubleEditPropertyManager, SIGNAL(valueChanged(QtProperty*,double)),
				this, SLOT(slotValueChanged(QtProperty*,double)));
	connect(doubleEditPropertyManager, SIGNAL(rangeChanged(QtProperty*,double,double,QString)),
				this, SLOT(slotRangeChanged(QtProperty*,double,double,QString)));
	connect(doubleEditPropertyManager, SIGNAL(decimalsChanged(QtProperty*,int)),
				this, SLOT(slotDecimalsChanged(QtProperty*,int)));
	// CoordinateIndexPropertyManager
	int coordinateIndexId = coordinateIndexTypeId();
	QtCoordinateIndexPropertyManager *coordinateIndexPropertyManager = new QtCoordinateIndexPropertyManager(this);
	d_ptr->m_typeToPropertyManager[coordinateIndexId] = coordinateIndexPropertyManager;
	d_ptr->m_typeToAttributeToAttributeType[coordinateIndexId][d_ptr->m_minimumAttribute] =
			QVariant::Int;
	d_ptr->m_typeToAttributeToAttributeType[coordinateIndexId][d_ptr->m_maximumAttribute] =
			QVariant::Int;
	d_ptr->m_typeToAttributeToAttributeType[coordinateIndexId][d_ptr->m_toolTipAttribute] =
			QVariant::String;
	d_ptr->m_typeToAttributeToAttributeType[coordinateIndexId][d_ptr->m_decimalsAttribute] =
			QVariant::Int;
	d_ptr->m_typeToAttributeToAttributeType[coordinateIndexId][d_ptr->m_coordinateVectorAttribute] =
			QVariant::Int;
	d_ptr->m_typeToAttributeToAttributeType[coordinateIndexId][d_ptr->m_coordianteSpaceTypeAttribute] =
			QVariant::Bool;
	d_ptr->m_typeToAttributeToAttributeType[coordinateIndexId][d_ptr->m_coordianteTimeUnitBaseAttribute] =
			QVariant::Int;
	d_ptr->m_typeToAttributeToAttributeType[coordinateIndexId][d_ptr->m_coordianteTimeUnitDisplayAttribute] =
			QVariant::Int;
	d_ptr->m_typeToValueType[coordinateIndexId] = QVariant::Int;
	connect(coordinateIndexPropertyManager, SIGNAL(valueChanged(QtProperty*,int)),
				this, SLOT(slotValueChanged(QtProperty*,int)));
	connect(coordinateIndexPropertyManager, SIGNAL(decimalsChanged(QtProperty*,int)),
				this, SLOT(slotDecimalsChanged(QtProperty*,int)));
	connect(coordinateIndexPropertyManager, SIGNAL(coordinateVectorChanged(QtProperty*,const QVector<double> &)),
				this, SLOT(slotVectorChanged(QtProperty*,const QVector<double> &)));
	connect(coordinateIndexPropertyManager, SIGNAL(coordinateSpaceTypeChanged(QtProperty*,bool)),
				this, SLOT(slotSpaceTypeChanged(QtProperty*,bool)));
	connect(coordinateIndexPropertyManager, SIGNAL(coordinateTimeUnitBaseChanged(QtProperty*,int)),
				this, SLOT(slotTimeUnitBaseChanged(QtProperty*,int)));
	connect(coordinateIndexPropertyManager, SIGNAL(coordinateTimeUnitDisplayChanged(QtProperty*,int)),
				this, SLOT(slotTimeUnitDisplayChanged(QtProperty*,int)));
	// BoolPropertyManager
	...
	connect(colorPropertyManager, SIGNAL(propertyRemoved(QtProperty*,QtProperty*)),
				this, SLOT(slotPropertyRemoved(QtProperty*,QtProperty*)));
	// DateTimeFormatPropertyManager
	QtDateTimeFormatPropertyManager *dateTimeFormatPropertyManager = new QtDateTimeFormatPropertyManager(this);
	int dateTimeFormatId = datTimeFormatTypeId();
	d_ptr->m_typeToPropertyManager[dateTimeFormatId] = dateTimeFormatPropertyManager;
	d_ptr->m_typeToValueType[dateTimeFormatId] = QVariant::StringList;
	connect(dateTimeFormatPropertyManager, SIGNAL(valueChanged(QtProperty*,QStringList)),
				this, SLOT(slotValueChanged(QtProperty*,QStringList)));
	connect(dateTimeFormatPropertyManager->subStringPropertyManager(), SIGNAL(valueChanged(QtProperty*,QString)),
				this, SLOT(slotValueChanged(QtProperty*,QString)));
	connect(dateTimeFormatPropertyManager, SIGNAL(propertyInserted(QtProperty*,QtProperty*,QtProperty*)),
				this, SLOT(slotPropertyInserted(QtProperty*,QtProperty*,QtProperty*)));
	connect(dateTimeFormatPropertyManager, SIGNAL(propertyRemoved(QtProperty*,QtProperty*)),
				this, SLOT(slotPropertyRemoved(QtProperty*,QtProperty*)));
	// EnumPropertyManager
...
QVariant QtVariantPropertyManager::value(const QtProperty *property) const
{
	...
	} else if (QtDoubleEditPropertyManager *doubleEditManager = qobject_cast<QtDoubleEditPropertyManager *>(manager)) {
		return doubleEditManager->value(internProp);
	} else if (QtCoordinateIndexPropertyManager *coordinateManager = qobject_cast<QtCoordinateIndexPropertyManager *>(manager)) {
		return coordinateManager->value(internProp);
	...
	} else if (QtDateTimeFormatPropertyManager *dateTimeFormatManager = qobject_cast<QtDateTimeFormatPropertyManager *>(manager)) {
		return dateTimeFormatManager->value(internProp);
...
QVariant QtVariantPropertyManager::attributeValue(const QtProperty *property, const QString &attribute) const
{
	...
	if (QtIntPropertyManager *intManager = qobject_cast<QtIntPropertyManager *>(manager)) {
		...
		if (attribute == d_ptr->m_maximumAttribute)
			return doubleEditManager->maximum(internProp);
		if (attribute == d_ptr->m_minimumAttribute)
			return doubleEditManager->minimum(internProp);
		if (attribute == d_ptr->m_toolTipAttribute)
			return doubleEditManager->toolTip(internProp);
		if (attribute == d_ptr->m_decimalsAttribute)
			return doubleEditManager->decimals(internProp);
		return QVariant();
	} else if (QtCoordinateIndexPropertyManager *coordinateManager = qobject_cast<QtCoordinateIndexPropertyManager *>(manager)) {
		if (attribute == d_ptr->m_toolTipAttribute)
			return coordinateManager->toolTip(internProp);
		if (attribute == d_ptr->m_decimalsAttribute)
			return coordinateManager->decimals(internProp);
		if (attribute == d_ptr->m_coordinateVectorAttribute)
			return QVariant::fromValue<QVector<double> >(coordinateManager->coordinateVector(internProp));
		if (attribute == d_ptr->m_coordianteSpaceTypeAttribute)
			return coordinateManager->spaceType(internProp);
		if (attribute == d_ptr->m_coordianteTimeUnitBaseAttribute)
			return coordinateManager->timeUnitBase(internProp);
		if (attribute == d_ptr->m_coordianteTimeUnitDisplayAttribute)
			return coordinateManager->timeUnitDisplay(internProp);
		return QVariant();
	} else if (QtStringPropertyManager *stringManager = qobject_cast<QtStringPropertyManager *>(manager)) {
...
void QtVariantPropertyManager::setValue(QtProperty *property, const QVariant &val)
{
	...
	if (QtIntPropertyManager *intManager = qobject_cast<QtIntPropertyManager *>(manager)) {
		...
		doubleEditManager->setValue(internProp, qvariant_cast<double>(val));
		return;
	} else if (QtCoordinateIndexPropertyManager *coordinateManager = qobject_cast<QtCoordinateIndexPropertyManager *>(manager)) {
		coordinateManager->setValue(internProp, qvariant_cast<int>(val));
		return;
	} else if (QtBoolPropertyManager *boolManager = qobject_cast<QtBoolPropertyManager *>(manager)) {
		...
		dateTimeFormatManager->setValue(internProp, qvariant_cast<QStringList>(val));
		return;
	} else if (QtEnumPropertyManager *enumManager = qobject_cast<QtEnumPropertyManager *>(manager)) {
		...
		const QString &attribute, const QVariant &value)
{
	...
	if (QtIntPropertyManager *intManager = qobject_cast<QtIntPropertyManager *>(manager)) {
		...
		if (attribute == d_ptr->m_maximumAttribute)
			doubleEditManager->setMaximum(internProp, qvariant_cast<double>(value));
		if (attribute == d_ptr->m_minimumAttribute)
			doubleEditManager->setMinimum(internProp, qvariant_cast<double>(value));
		if (attribute == d_ptr->m_toolTipAttribute)
			doubleEditManager->setToolTip(internProp, qvariant_cast<QString>(value));
		if (attribute == d_ptr->m_decimalsAttribute)
			doubleEditManager->setDecimals(internProp, qvariant_cast<int>(value));
		return;
	} else if (QtCoordinateIndexPropertyManager *coordinateManager = qobject_cast<QtCoordinateIndexPropertyManager *>(manager)) {
		if (attribute == d_ptr->m_toolTipAttribute)
			coordinateManager->setToolTip(internProp, qvariant_cast<QString>(value));
		if (attribute == d_ptr->m_decimalsAttribute)
			coordinateManager->setDecimals(internProp, qvariant_cast<int>(value));
		if (attribute == d_ptr->m_coordinateVectorAttribute)
			coordinateManager->setCoordinateVector(internProp, qvariant_cast<QVector<double>>(value));
		if (attribute == d_ptr->m_coordianteSpaceTypeAttribute)
			coordinateManager->setSpaceType(internProp, qvariant_cast<bool>(value));
		if (attribute == d_ptr->m_coordianteTimeUnitBaseAttribute)
			coordinateManager->setTimeUnitBase(internProp, qvariant_cast<int>(value));
		if (attribute == d_ptr->m_coordianteTimeUnitDisplayAttribute)
			coordinateManager->setTimeUnitDisplay(internProp, qvariant_cast<int>(value));
		return;
	} else if (QtStringPropertyManager *stringManager = qobject_cast<QtStringPropertyManager *>(manager)) {
		if (attribute == d_ptr->m_regExpAttribute)
...
public:
	...
	QtDoubleLineEditFactory			*m_doubleLineEditFactory;
	QtDateTimeFormatEditorFactory	*m_dateTimeFormatEditorFactory;
	QtCoordinateIndexEditFactory	*m_coordinateIndexEditfactory;
	...
	: QtAbstractEditorFactory<QtVariantPropertyManager>(parent), d_ptr(new QtVariantEditorFactoryPrivate())
{
	...
	d_ptr->m_dateTimeFormatEditorFactory = new QtDateTimeFormatEditorFactory(this);
	const int dateTimeFormatId = QtVariantPropertyManager::datTimeFormatTypeId();
	d_ptr->m_factoryToType[d_ptr->m_dateTimeFormatEditorFactory] = dateTimeFormatId;
	d_ptr->m_typeToFactory[dateTimeFormatId] = d_ptr->m_dateTimeFormatEditorFactory;
	...
	d_ptr->m_doubleLineEditFactory = new QtDoubleLineEditFactory(this);
	const int doubleEditId = QtVariantPropertyManager::doubleEditTypeId();
	d_ptr->m_factoryToType[d_ptr->m_doubleLineEditFactory] = doubleEditId;
	d_ptr->m_typeToFactory[doubleEditId] = d_ptr->m_doubleLineEditFactory;

	d_ptr->m_coordinateIndexEditfactory = new QtCoordinateIndexEditFactory(this);
	const int coordinateIndexId = QtVariantPropertyManager::coordinateIndexTypeId();
	d_ptr->m_factoryToType[d_ptr->m_coordinateIndexEditfactory] = coordinateIndexId;
	d_ptr->m_typeToFactory[coordinateIndexId] = d_ptr->m_coordinateIndexEditfactory;
...
void QtVariantEditorFactory::connectPropertyManager(QtVariantPropertyManager *manager)
{
	...
	QList<QtDateTimeFormatPropertyManager *> dateTimeFormatPropertyManagers = manager->findChildren<QtDateTimeFormatPropertyManager *>();
	QListIterator<QtDateTimeFormatPropertyManager *> itDateTimeFormat(dateTimeFormatPropertyManagers);
	while (itDateTimeFormat.hasNext()) {
		QtDateTimeFormatPropertyManager *manager = itDateTimeFormat.next();
		d_ptr->m_dateTimeFormatEditorFactory->addPropertyManager(manager);
	...
	QList<QtDoubleEditPropertyManager *> doubleEditPropertyManagers = manager->findChildren<QtDoubleEditPropertyManager *>();
	QListIterator<QtDoubleEditPropertyManager *> itDoubleEdit(doubleEditPropertyManagers);
	while (itDoubleEdit.hasNext())
		d_ptr->m_doubleLineEditFactory->addPropertyManager(itDoubleEdit.next());

	QList<QtCoordinateIndexPropertyManager *> coordinatePropertyManagers = manager->findChildren<QtCoordinateIndexPropertyManager *>();
	QListIterator<QtCoordinateIndexPropertyManager *> itCoordinateIndex(coordinatePropertyManagers);
	while (itCoordinateIndex.hasNext())
		d_ptr->m_coordinateIndexEditfactory->addPropertyManager(itCoordinateIndex.next());
...
void QtVariantEditorFactory::disconnectPropertyManager(QtVariantPropertyManager *manager)
{
	...
	QListIterator<QtDoubleEditPropertyManager *> itDoubleEdit(doubleEditPropertyManagers);
	while (itDoubleEdit.hasNext())
		d_ptr->m_doubleLineEditFactory->removePropertyManager(itDoubleEdit.next());

	QList<QtCoordinateIndexPropertyManager *> coordinatePropertyManagers = manager->findChildren<QtCoordinateIndexPropertyManager *>();
	QListIterator<QtCoordinateIndexPropertyManager *> itCoordinate(coordinatePropertyManagers);
	while (itCoordinate.hasNext())
		d_ptr->m_coordinateIndexEditfactory->removePropertyManager(itCoordinate.next());
