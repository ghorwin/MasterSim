...
public:
	...
	static QIcon brushValueIcon(const QBrush &b);
	  static QPixmap penValuePixmap(const QPen &p);
	  static QIcon penValueIcon(const QPen &p);
	  static QPixmap markerValuePixmap(const int &m);
	  static QIcon markerValueIcon(const int &m);
...
// only for Qt4, Qt 5 has an own class
#if QT_VERSION < 0x050000
