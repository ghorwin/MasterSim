...
def configCommandLineArguments():
	...
	parser.add_argument('--omp', dest='use_omp', required=False, action='store_true', default=False,
	                    help='Enables OpenMP compilation if set.')
...
try:
	...
	if not args.skip_build:
		if not args.silent:
			if sys.version_info >= (3, 0):
				input("Configuration done, press any key to start build!");
			else:
				print("Configuration done, press any key to start build!");
				raw_input()
		buildApplication(args.build_script, args.x64, args.use_omp)
	...
	if not args.skip_deployment:
		...
		if not args.silent:
			if sys.version_info >= (3, 0):
				input("Build done, press any key to start packaging!");
			else:
				print("Build done, press any key to start packaging!");
				raw_input()
