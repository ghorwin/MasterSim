...
class QtDoubleEditPropertyManager : public QtAbstractPropertyManager
{
	Q_OBJECT
public:
	QtDoubleEditPropertyManager(QObject *parent = 0);
	~QtDoubleEditPropertyManager();

	double value(const QtProperty *property) const;
	double minimum(const QtProperty *property) const;
	double maximum(const QtProperty *property) const;
	QString toolTip(const QtProperty *property) const;
	int decimals(const QtProperty *property) const;
...
public Q_SLOTS:
	...
	void setMinimum(QtProperty *property, double minVal);
	void setMaximum(QtProperty *property, double maxVal);
	void setToolTip(QtProperty *property, QString toolTip);
	void setRange(QtProperty *property, double minVal, double maxVal);
	void setDecimals(QtProperty *property, int prec);
Q_SIGNALS:
	void valueChanged(QtProperty *property, double val);
	void rangeChanged(QtProperty *property, double minVal, double maxVal, QString toolTip);
	void decimalsChanged(QtProperty *property, int prec);
protected:
	QString valueText(const QtProperty *property) const;
	virtual void initializeProperty(QtProperty *property);
	virtual void uninitializeProperty(QtProperty *property);
private:
	QScopedPointer<QtDoubleEditPropertyManagerPrivate> d_ptr;
	Q_DECLARE_PRIVATE(QtDoubleEditPropertyManager)
	Q_DISABLE_COPY(QtDoubleEditPropertyManager)
};

class QtCoordinateIndexPropertyManagerPrivate;

class QtCoordinateIndexPropertyManager : public QtAbstractPropertyManager
{
	Q_OBJECT
public:
	QtCoordinateIndexPropertyManager(QObject *parent = 0);
	~QtCoordinateIndexPropertyManager();

	int value(const QtProperty *property) const;  // index of coordinate vector
	QString toolTip(const QtProperty *property) const;
	int decimals(const QtProperty *property) const;
	QVector<double>	coordinateVector(const QtProperty *property) const;
	bool spaceType(const QtProperty *property) const;
	int timeUnitBase(const QtProperty *property) const;
	int timeUnitDisplay(const QtProperty *property) const;

public Q_SLOTS:
	void setValue(QtProperty *property, int val);	// set index
	void setToolTip(QtProperty *property, QString toolTip);
	void setDecimals(QtProperty *property, int prec);
	void setCoordinateVector(QtProperty *property, QVector<double>);
	void setSpaceType(QtProperty *property, bool);
	void setTimeUnitBase(QtProperty *property, int);
	void setTimeUnitDisplay(QtProperty *property, int);
Q_SIGNALS:
	void valueChanged(QtProperty *property, int val);
	void decimalsChanged(QtProperty *property, int prec);
	void coordinateVectorChanged(QtProperty *property, const QVector<double> & coordinates);
	void coordinateSpaceTypeChanged(QtProperty *property, bool spaceType);
	void coordinateTimeUnitBaseChanged(QtProperty *property, int baseUnit);
	void coordinateTimeUnitDisplayChanged(QtProperty *property, int displayUnit);
protected:
	QString valueText(const QtProperty *property) const;
	virtual void initializeProperty(QtProperty *property);
	virtual void uninitializeProperty(QtProperty *property);
private:
	QScopedPointer<QtCoordinateIndexPropertyManagerPrivate> d_ptr;
	Q_DECLARE_PRIVATE(QtCoordinateIndexPropertyManager)
	Q_DISABLE_COPY(QtCoordinateIndexPropertyManager)
};
...
class QtDateTimeFormatPropertyManager : public QtAbstractPropertyManager
{
	Q_OBJECT
public:
	QtDateTimeFormatPropertyManager(QObject *parent = 0);
	~QtDateTimeFormatPropertyManager();
...
protected:
	...
	virtual void uninitializeProperty(QtProperty *property);
private:
	QScopedPointer<QtDateTimeFormatPropertyManagerPrivate> d_ptr;
	Q_DECLARE_PRIVATE(QtDateTimeFormatPropertyManager)
	Q_DISABLE_COPY(QtDateTimeFormatPropertyManager)
