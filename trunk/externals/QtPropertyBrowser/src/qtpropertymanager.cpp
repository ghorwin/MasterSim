...
{
	typename QMap<const QtProperty *, PrivateData>::const_iterator it = propertyMap.constFind(property);
...
template <class Value, class PrivateData>
static Value getToolTip(const QMap<const QtProperty *, PrivateData> &propertyMap,
			const QtProperty *property, const Value &defaultValue = Value())
{
	return getData<Value>(propertyMap, &PrivateData::m_toolTip, property, defaultValue);
}
...
{
	typename QMap<const QtProperty *, Value>::iterator it = propertyMap.find(property);
...
{
	typedef typename PropertyManagerPrivate::PropertyValueMap  ValueMap;
	typename ValueMap::iterator it = managerPrivate->m_values.find(property);
...
static void setBorderValues(PropertyManager *manager, PropertyManagerPrivate *managerPrivate,
			void (PropertyManager::*propertyChangedSignal)(QtProperty *),
			void (PropertyManager::*valueChangedSignal)(QtProperty *, ValueChangeParameter),
			void (PropertyManager::*rangeChangedSignal)(QtProperty *, ValueChangeParameter, ValueChangeParameter, QString),
			QtProperty *property, const Value &minVal, const Value &maxVal,
			void (PropertyManagerPrivate::*setSubPropertyRange)(QtProperty *,
					ValueChangeParameter, ValueChangeParameter, ValueChangeParameter))
{
...
//    auto &data = it.value();
	...
	emit (manager->*rangeChangedSignal)(property, data.minVal, data.maxVal, data.m_toolTip);
...
template <class PropertyManagerPrivate, class PrivateData>
static void setToolTipValue(PropertyManagerPrivate *managerPrivate,
			QtProperty *property, const QString &newToolTip)
{
	typedef typename PropertyManagerPrivate::PropertyValueMap  ValueMap;
	typename ValueMap::iterator it = managerPrivate->m_values.find(property);
	if (it == managerPrivate->m_values.end())
		return;
	PrivateData &data = it.value();
	if (data.m_toolTip == newToolTip)
		return;
	data.m_toolTip = newToolTip;
}
...
QString QtDoublePropertyManager::valueText(const QtProperty *property) const
{
	...
	if (it == d_ptr->m_values.constEnd())
		return QString();
	return QString("%L1").arg((double)it.value().val, 0, 'f', it.value().decimals);
}
...
// QtDoubleEditPropertyManager

class QtDoubleEditPropertyManagerPrivate
{
	QtDoubleEditPropertyManager *q_ptr;
	Q_DECLARE_PUBLIC(QtDoubleEditPropertyManager)
public:

	struct Data
	{
		Data() : val(0), minVal(-DBL_MAX), maxVal(DBL_MAX), decimals(2) {}
		double val;
		double minVal;
		double maxVal;
		int decimals;
		...
		void setMinimumValue(double newMinVal) { setSimpleMinimumData(this, newMinVal); }
		void setMaximumValue(double newMaxVal) { setSimpleMaximumData(this, newMaxVal); }
		void setToolTip(QString newToolTip) { m_toolTip = newToolTip; }
//		void setValue(double val) { m_val = val;  }
//		double value() const { return m_val;  }
	};

	typedef QMap<const QtProperty *, Data> PropertyValueMap;
	PropertyValueMap m_values1;
};

/*!
	\class QtDoubleEditPropertyManager
	\internal
	\inmodule QtDesigner
	\since 4.4

	\brief The QtDoubleEditPropertyManager provides and manages double properties.
	...
	In addition, QtDoubleEditPropertyManager provides the valueChanged() signal
	which is emitted whenever a property created by this manager
	changes, and the rangeChanged() signal which is emitted whenever
	such a property changes its range of valid values.
...
/*!
	\fn void QtDoubleEditPropertyManager::valueChanged(QtProperty *property, double value)
...
/*!
	\fn void QtDoubleEditPropertyManager::rangeChanged(QtProperty *property, double minimum, double maximum, QString toolTip)

	This signal is emitted whenever a property created by this manager
	changes its range of valid values, passing a pointer to the
	\a property and the new \a minimum and \a maximum values as well as a \a toolTip
...
/*!
	Creates a manager with the given \a parent.
*/
QtDoubleEditPropertyManager::QtDoubleEditPropertyManager(QObject *parent)
	: QtAbstractPropertyManager(parent), d_ptr(new QtDoubleEditPropertyManagerPrivate)
{
	d_ptr->q_ptr = this;
}
...
/*!
	Destroys  this manager, and all the properties it has created.
*/
QtDoubleEditPropertyManager::~QtDoubleEditPropertyManager()
{
	clear();
}
...
/*!
	...
	\sa setValue()
*/
double QtDoubleEditPropertyManager::value(const QtProperty *property) const
{
	return getValue<double>(d_ptr->m_values1, property, 0.0);
}
...
/*!
	...
	\sa maximum(), setRange()
*/
double QtDoubleEditPropertyManager::minimum(const QtProperty *property) const
{
	return getMinimum<double>(d_ptr->m_values1, property, 0.0);
}
...
/*!
	...
	\sa minimum(), setRange()
*/
double QtDoubleEditPropertyManager::maximum(const QtProperty *property) const
{
	return getMaximum<double>(d_ptr->m_values1, property, 0.0);
}

/*!
	Returns the given \a property's tool tip.

	\sa setRange()
*/
QString QtDoubleEditPropertyManager::toolTip(const QtProperty *property) const
{
	return getToolTip<QString>(d_ptr->m_values1, property, "");
}
...
/*!
	...
	\sa setDecimals()
*/
int QtDoubleEditPropertyManager::decimals(const QtProperty *property) const
{
	return getData<int>(d_ptr->m_values1, &QtDoubleEditPropertyManagerPrivate::Data::decimals, property, 0);
}

/*!
	\reimp
*/
QString QtDoubleEditPropertyManager::valueText(const QtProperty *property) const
{
	const QtDoubleEditPropertyManagerPrivate::PropertyValueMap::const_iterator it = d_ptr->m_values1.constFind(property);
//	const QtDoubleEditPropertyManagerPrivate::PropertyValueMap& vmap = d_ptr->m_values1;
	if (it == d_ptr->m_values1.constEnd())
		return QString();
	///< \todo Create formating
	QString text = QString("%L1").arg(it.value().val, 0, 'g');
	return text;
}

/*!
	\fn void QtDoubleEditPropertyManager::setValue(QtProperty *property, double value)
	...
	\sa value(), setRange(), valueChanged()
*/
void QtDoubleEditPropertyManager::setValue(QtProperty *property, double val)
{
		typedef typename QtDoubleEditPropertyManagerPrivate::PropertyValueMap  ValueMap;
		typename ValueMap::iterator it = d_ptr.data()->m_values1.find(property);

		if (it == d_ptr.data()->m_values1.end())
			return;

		typedef typename QtDoubleEditPropertyManagerPrivate::Data PrivateData;
		PrivateData &data = it.value();

		if (data.val == val)
			return;

		const double oldVal = data.val;
...
/*!
	\fn void QtDoubleEditPropertyManager::setDecimals(QtProperty *property, int prec)
	...
	\sa decimals()
*/
void QtDoubleEditPropertyManager::setDecimals(QtProperty *property, int prec)
{
	const QtDoubleEditPropertyManagerPrivate::PropertyValueMap::iterator it = d_ptr->m_values1.find(property);
	if (it == d_ptr->m_values1.end())
		return;

	QtDoubleEditPropertyManagerPrivate::Data data = it.value();
...
/*!
	...
	\sa minimum(), setRange(), rangeChanged()
*/
void QtDoubleEditPropertyManager::setMinimum(QtProperty *property, double minVal)
{
	typedef typename QtDoubleEditPropertyManagerPrivate::PropertyValueMap  ValueMap;
	typename ValueMap::iterator it = d_ptr.data()->m_values1.find(property);
	if (it == d_ptr.data()->m_values1.end())
	...
	emit rangeChanged(property, data.minVal, data.maxVal, data.m_toolTip);
...
/*!
	...
	\sa maximum(), setRange(), rangeChanged()
*/
void QtDoubleEditPropertyManager::setMaximum(QtProperty *property, double maxVal)
{
	typedef typename QtDoubleEditPropertyManagerPrivate::PropertyValueMap  ValueMap;
	typename ValueMap::iterator it = d_ptr.data()->m_values1.find(property);
	if (it == d_ptr.data()->m_values1.end())
...
/*!
	Sets the tool tip for the given \a property to \a toolTip.

	\sa setRange(), rangeChanged()
*/
void QtDoubleEditPropertyManager::setToolTip(QtProperty *property, QString toolTip)
{
	typedef typename QtDoubleEditPropertyManagerPrivate::PropertyValueMap  ValueMap;
	typename ValueMap::iterator it = d_ptr.data()->m_values1.find(property);
	if (it == d_ptr.data()->m_values1.end())
	...
	QtDoubleEditPropertyManagerPrivate::Data &data = it.value();
	if (data.m_toolTip == toolTip)
		return;

	data.m_toolTip = toolTip;
}

/*!
	\fn void QtDoubleEditPropertyManager::setRange(QtProperty *property, double minimum, double maximum)
	...
	\sa setMinimum(), setMaximum(), rangeChanged()
*/
void QtDoubleEditPropertyManager::setRange(QtProperty *property, double minVal, double maxVal)
{
...
/*!
	\reimp
*/
void QtDoubleEditPropertyManager::initializeProperty(QtProperty *property)
{
	d_ptr->m_values1[property] = QtDoubleEditPropertyManagerPrivate::Data();
}

/*!
	\reimp
*/
void QtDoubleEditPropertyManager::uninitializeProperty(QtProperty *property)
{
	d_ptr->m_values1.remove(property);
}
// QtCoordinateIndexPropertyManager

class QtCoordinateIndexPropertyManagerPrivate {
	QtCoordinateIndexPropertyManager *q_ptr;
	Q_DECLARE_PUBLIC(QtCoordinateIndexPropertyManager)
public:
	...
	struct Data {
		...
		bool			m_spaceType;
		int				m_timeUnitBase;
		int				m_timeUnitDisplay;
		QVector<double> coordinates() const { return m_coordinates; }
		bool spaceType() const { return m_spaceType; }
		int timeUnitBase() const { return m_timeUnitBase; }
		int timeUnitDisplay() const { return m_timeUnitDisplay; }
		QString toolTip() const { return m_toolTip; }
		void setToolTip(QString newToolTip) { m_toolTip = newToolTip; }
		void setCoordinateVector(QVector<double> coordinates) { m_coordinates = coordinates; }
		void setSpaceType(bool spaceType) { m_spaceType = spaceType; }
		void setTimeUnitBase(int baseUnit) { m_timeUnitBase = baseUnit; }
		void setTimeUnitDisplay(int displayUnit) { m_timeUnitDisplay = displayUnit; }
	};
...
/*!
	\class QtCoordinateIndexPropertyManager
	\internal

	\brief The QtCoordinateIndexPropertyManager provides and manages coordinate properties.

	A coordinate property has a current value which must be part of a coordinate vector.
	A range specifying the max and min index of this vector.
	...
	Alternatively, the range can be defined in one go using the
	setRange() slot.
	The coordinate vector must be set before any use of this property.

	In addition, QtCoordinateIndexPropertyManager provides the valueChanged() signal
	which is emitted whenever a property created by this manager
	changes, and the rangeChanged() signal which is emitted whenever
	such a property changes its range of valid values.
...
/*!
	\fn void QtCoordinateIndexPropertyManagerv::valueChanged(QtProperty *property, int value)
...
/*!
	\fn void QtCoordinateIndexPropertyManager::rangeChanged(QtProperty *property, int minimum, int maximum, QString toolTip)
...
/*!
	\fn void QtCoordinateIndexPropertyManager::decimalsChanged(QtProperty *property, int prec)
...
/*!
	Creates a manager with the given \a parent.
*/
QtCoordinateIndexPropertyManager::QtCoordinateIndexPropertyManager(QObject *parent)
	: QtAbstractPropertyManager(parent),
	  d_ptr(new QtCoordinateIndexPropertyManagerPrivate)
{
	d_ptr->q_ptr = this;
}
...
/*!
	Destroys  this manager, and all the properties it has created.
*/
QtCoordinateIndexPropertyManager::~QtCoordinateIndexPropertyManager()
{
	clear();
}
...
/*!
	...
	\sa setValue()
*/
int QtCoordinateIndexPropertyManager::value(const QtProperty *property) const
{
	return getValue<int>(d_ptr->m_values1, property, 0);
}
...
/*!
	...
	\sa setRange()
*/
QString QtCoordinateIndexPropertyManager::toolTip(const QtProperty *property) const
{
	return getToolTip<QString>(d_ptr->m_values1, property, "");
}
...
/*!
	...
	\sa setDecimals()
*/
int QtCoordinateIndexPropertyManager::decimals(const QtProperty *property) const
{
	return getData<int>(d_ptr->m_values1, &QtCoordinateIndexPropertyManagerPrivate::Data::decimals, property, 0);
}

QVector<double>	QtCoordinateIndexPropertyManager::coordinateVector(const QtProperty *property) const {
	return getData<QVector<double> >(d_ptr->m_values1, &QtCoordinateIndexPropertyManagerPrivate::Data::m_coordinates, property, QVector<double>());
}

/*!
	Returns the given \a property's coordinate type. If true it is space otherwise time..

	\sa setSpaceType()
*/
bool QtCoordinateIndexPropertyManager::spaceType(const QtProperty *property) const
{
	return getData<bool>(d_ptr->m_values1, &QtCoordinateIndexPropertyManagerPrivate::Data::m_spaceType, property, true);
}

/*!
	Returns the given \a property's time unit of coordinate vector.

	\sa setTimeUnitBase()
*/
int QtCoordinateIndexPropertyManager::timeUnitBase(const QtProperty *property) const
{
	return getData<int>(d_ptr->m_values1, &QtCoordinateIndexPropertyManagerPrivate::Data::m_timeUnitBase, property, 0);
}

/*!
	Returns the given \a property's time unit for display in edit.

	\sa setTimeUnitDisplay()
*/
int QtCoordinateIndexPropertyManager::timeUnitDisplay(const QtProperty *property) const
{
	return getData<int>(d_ptr->m_values1, &QtCoordinateIndexPropertyManagerPrivate::Data::m_timeUnitDisplay, property, 3);
}

/*!
	\reimp
*/
QString QtCoordinateIndexPropertyManager::valueText(const QtProperty *property) const
{
	const QtCoordinateIndexPropertyManagerPrivate::PropertyValueMap::const_iterator it = d_ptr->m_values1.constFind(property);
	if (it == d_ptr->m_values1.constEnd())
		return QString();
	if(it->m_coordinates.isEmpty())
		return QString();

	///< \todo Create formating
	int index = it.value().val;
	double value = 0.0;
	if(index < it->m_coordinates.size())
		value = it->m_coordinates[index];
	QString unit;
	if(it->m_spaceType) {
		unit = "m";
	}
	else {
		switch(it->m_timeUnitDisplay) {
			case 0: unit = "s"; break;
			case 1: unit = "min"; break;
			case 2: unit = "h"; break;
			case 3: unit = "d"; break;
			case 4: unit = "a"; break;
			default: break;
		}
		value = QPW::CoordinateIndexEdit::timeConvert(static_cast<QPW::CoordinateIndexEdit::TimeUnit>(it->m_timeUnitBase),
													  static_cast<QPW::CoordinateIndexEdit::TimeUnit>(it->m_timeUnitDisplay)
													  , value);
	}
	QString text = QString("%1 - %L2 %3").arg(index).arg(value, 0, 'g').arg(unit);
	return text;
}

/*!
	\fn void QtCoordinateIndexPropertyManager::setValue(QtProperty *property, int value)

	Sets the value of the given \a property to \a value.
	For this property the value is an index of a given coordinate vector
	...
	\sa value(), setRange(), valueChanged()
*/
void QtCoordinateIndexPropertyManager::setValue(QtProperty *property, int val)
{
		typedef typename QtCoordinateIndexPropertyManagerPrivate::PropertyValueMap  ValueMap;
		typename ValueMap::iterator it = d_ptr.data()->m_values1.find(property);

		if (it == d_ptr.data()->m_values1.end())
			return;

		typedef typename QtCoordinateIndexPropertyManagerPrivate::Data PrivateData;
		PrivateData &data = it.value();
		...
		// Mind: data.m_coordinates can be empty!
		if (data.m_coordinates.empty())
			return; // no change

		data.val = qBound<unsigned int>(0, (unsigned int)val, data.m_coordinates.size()-1);
...
/*!
	\fn void QtCoordinateIndexPropertyManager::setDecimals(QtProperty *property, int prec)
	...
	\sa decimals()
*/
void QtCoordinateIndexPropertyManager::setDecimals(QtProperty *property, int prec)
{
	const QtCoordinateIndexPropertyManagerPrivate::PropertyValueMap::iterator it = d_ptr->m_values1.find(property);
	if (it == d_ptr->m_values1.end())
		return;

	QtCoordinateIndexPropertyManagerPrivate::Data data = it.value();
...
/*!
	...
	\sa setRange(), rangeChanged()
*/
void QtCoordinateIndexPropertyManager::setToolTip(QtProperty *property, QString toolTip)
{
	typedef typename QtCoordinateIndexPropertyManagerPrivate::PropertyValueMap  ValueMap;
	typename ValueMap::iterator it = d_ptr.data()->m_values1.find(property);
	if (it == d_ptr.data()->m_values1.end())
...
/*!
	Sets the coordinate vector for the given \a property to \a coordinates.

	The coordinate vector maps an index to a certain coordinate.
	This can be space or time according \a m_spaceType.

	\sa spaceType(), timeUnitBase(), timeUnitDisplay()
*/
void QtCoordinateIndexPropertyManager::setCoordinateVector(QtProperty *property, QVector<double> coordinates)
{
	typedef typename QtCoordinateIndexPropertyManagerPrivate::PropertyValueMap  ValueMap;
	typename ValueMap::iterator it = d_ptr.data()->m_values1.find(property);
	if (it == d_ptr.data()->m_values1.end())
	...
	const int oldVal = data.val;

	data.setCoordinateVector(coordinates);
	emit coordinateVectorChanged(property, coordinates);
	emit propertyChanged(property);
...
/*!
	Sets the type of coordinate vector for the given \a property to \a spaceType.

	If true the coordinates represents space in [m] otherwise time in m_timeUnitBase.

	\sa spaceType(), timeUnitBase(), timeUnitDisplay()
*/
void QtCoordinateIndexPropertyManager::setSpaceType(QtProperty *property, bool spaceType)
{
	typedef typename QtCoordinateIndexPropertyManagerPrivate::PropertyValueMap  ValueMap;
	typename ValueMap::iterator it = d_ptr.data()->m_values1.find(property);
	if (it == d_ptr.data()->m_values1.end())
	...
	if (data.spaceType() == spaceType)
		return;

	data.setSpaceType(spaceType);
	emit coordinateSpaceTypeChanged(property, spaceType);
...
/*!
	Sets the time unit for time type coordinate vector for the given \a property to \a timeUnitBase.

	\sa spaceType(), timeUnitBase(), timeUnitDisplay()
*/
void QtCoordinateIndexPropertyManager::setTimeUnitBase(QtProperty *property, int timeUnitBase)
{
	typedef typename QtCoordinateIndexPropertyManagerPrivate::PropertyValueMap  ValueMap;
	typename ValueMap::iterator it = d_ptr.data()->m_values1.find(property);
	if (it == d_ptr.data()->m_values1.end())
	...
	if (data.timeUnitBase() == timeUnitBase)
		return;

	data.setTimeUnitBase(timeUnitBase);
	emit coordinateTimeUnitBaseChanged(property, timeUnitBase);
...
/*!
	Sets the time unit for displaying time in edit for the given \a property to \a timeUnitBase.

	\sa spaceType(), timeUnitBase(), timeUnitDisplay()
*/
void QtCoordinateIndexPropertyManager::setTimeUnitDisplay(QtProperty *property, int timeUnitDisplay)
{
	typedef typename QtCoordinateIndexPropertyManagerPrivate::PropertyValueMap  ValueMap;
	typename ValueMap::iterator it = d_ptr.data()->m_values1.find(property);
	if (it == d_ptr.data()->m_values1.end())
	...
	if (data.timeUnitDisplay() == timeUnitDisplay)
		return;

	data.setTimeUnitDisplay(timeUnitDisplay);
	emit coordinateTimeUnitDisplayChanged(property, timeUnitDisplay);
...
/*!
	\reimp
*/
void QtCoordinateIndexPropertyManager::initializeProperty(QtProperty *property)
{
	d_ptr->m_values1[property] = QtCoordinateIndexPropertyManagerPrivate::Data();
}

/*!
	\reimp
*/
void QtCoordinateIndexPropertyManager::uninitializeProperty(QtProperty *property)
{
	d_ptr->m_values1.remove(property);
}
...
//	static const QString falseText = tr("False");
	// Hack by Andreas: to avoid translating "true" to "wahr", just leave this empty
	//                  after all we have a checkbox that should be understandable by everyone, right?
	return QString(); // it.value() ? trueText : falseText;
}
...
class QtDateTimeFormatPropertyManagerPrivate
{
	QtDateTimeFormatPropertyManager *q_ptr;
	Q_DECLARE_PUBLIC(QtDateTimeFormatPropertyManager)
public:
	QtDateTimeFormatPropertyManagerPrivate();

	void slotStringChanged(QtProperty *property, const QString& value);
	void slotPropertyDestroyed(QtProperty *property);

	typedef QMap<const QtProperty *, QStringList> PropertyValueMap;
	PropertyValueMap m_values;

	QtStringPropertyManager *m_stringPropertyManager;
	QStringList				m_dateFormats;
	QStringList				m_dateFormatsDefault;

	QMap<const QtProperty *, QtProperty *> m_propertyToMS;
	QMap<const QtProperty *, QtProperty *> m_propertyToS;
	QMap<const QtProperty *, QtProperty *> m_propertyToMi;
	QMap<const QtProperty *, QtProperty *> m_propertyToH;
	QMap<const QtProperty *, QtProperty *> m_propertyToD;
	QMap<const QtProperty *, QtProperty *> m_propertyToW;
	QMap<const QtProperty *, QtProperty *> m_propertyToMo;
	QMap<const QtProperty *, QtProperty *> m_propertyToY;

	QMap<const QtProperty *, QtProperty *> m_MSToProperty;
	QMap<const QtProperty *, QtProperty *> m_SToProperty;
	QMap<const QtProperty *, QtProperty *> m_MiToProperty;
	QMap<const QtProperty *, QtProperty *> m_HToProperty;
	QMap<const QtProperty *, QtProperty *> m_DToProperty;
	QMap<const QtProperty *, QtProperty *> m_WToProperty;
	QMap<const QtProperty *, QtProperty *> m_MoToProperty;
	QMap<const QtProperty *, QtProperty *> m_YToProperty;
};

QtDateTimeFormatPropertyManagerPrivate::QtDateTimeFormatPropertyManagerPrivate() {
	m_dateFormats << "hh:mm:ss:zzz\nddd dd MMM yyyy";
	m_dateFormats << "hh:mm:ss\nddd dd MMM yyyy";
	m_dateFormats << "hh:mm\nddd dd MMM yyyy";
	m_dateFormats << "hh:mm\nddd dd MMM yyyy";
	m_dateFormats << "ddd dd MMM yyyy";
	m_dateFormats << "dd MMM yyyy";
	m_dateFormats << "MMM yyyy";
	m_dateFormats << "yyyy";

	m_dateFormatsDefault = m_dateFormats;
}

void QtDateTimeFormatPropertyManagerPrivate::slotStringChanged(QtProperty *property, const QString& value)
{
	if (QtProperty *prop = m_MSToProperty.value(property, 0)) {
		QStringList s = m_values[prop];
		s[0] = value;
		q_ptr->setValue(prop, s);
		...
		s[1] = value;
		q_ptr->setValue(prop, s);
	} else if (QtProperty *prop = m_MiToProperty.value(property, 0)) {
		QStringList s = m_values[prop];
		s[2] = value;
		q_ptr->setValue(prop, s);
	} else if (QtProperty *prop = m_HToProperty.value(property, 0)) {
		...
		s[3] = value;
		q_ptr->setValue(prop, s);
	} else if (QtProperty *prop = m_DToProperty.value(property, 0)) {
		QStringList s = m_values[prop];
		s[4] = value;
		q_ptr->setValue(prop, s);
	} else if (QtProperty *prop = m_WToProperty.value(property, 0)) {
		...
		s[5] = value;
		q_ptr->setValue(prop, s);
	} else if (QtProperty *prop = m_MoToProperty.value(property, 0)) {
		QStringList s = m_values[prop];
		s[6] = value;
		q_ptr->setValue(prop, s);
	} else if (QtProperty *prop = m_YToProperty.value(property, 0)) {
...
void QtDateTimeFormatPropertyManagerPrivate::slotPropertyDestroyed(QtProperty *property)
{
	if (QtProperty *pointProp = m_MSToProperty.value(property, 0)) {
		m_propertyToMS[pointProp] = 0;
		m_MSToProperty.remove(property);
	} else if (QtProperty *pointProp = m_SToProperty.value(property, 0)) {
		m_propertyToS[pointProp] = 0;
		m_SToProperty.remove(property);
	} else if (QtProperty *pointProp = m_MiToProperty.value(property, 0)) {
		m_propertyToMi[pointProp] = 0;
		m_MiToProperty.remove(property);
	} else if (QtProperty *pointProp = m_HToProperty.value(property, 0)) {
		m_propertyToH[pointProp] = 0;
		m_HToProperty.remove(property);
	} else if (QtProperty *pointProp = m_DToProperty.value(property, 0)) {
		m_propertyToD[pointProp] = 0;
		m_DToProperty.remove(property);
	} else if (QtProperty *pointProp = m_WToProperty.value(property, 0)) {
		m_propertyToW[pointProp] = 0;
		m_WToProperty.remove(property);
	} else if (QtProperty *pointProp = m_MoToProperty.value(property, 0)) {
		m_propertyToMo[pointProp] = 0;
		m_MoToProperty.remove(property);
	} else if (QtProperty *pointProp = m_YToProperty.value(property, 0)) {
		m_propertyToY[pointProp] = 0;
		m_YToProperty.remove(property);
	}
...
/*!
	\class QtDateTimeFormatPropertyManager
	\internal

	\brief The QtDateTimeFormatPropertyManager provides and manages DateTime format specification properties.

	A DateTime format specification is give as format string for 8 different interval types.
	\li Millisecond
	\li Second
	\li Minute
	\li Hour
	...
	\li Month
	\li Year
	Each format string is represented by a string property

	The subproperties are created by a QtStringPropertyManager object. This
	manager can be retrieved using the subStringPropertyManager() function.  In
	order to provide editing widgets for the subproperties in a
	property browser widget, this manager must be associated with an
	editor factory.

	In addition, QtDateTimeFormatPropertyManager provides the valueChanged() signal
	which is emitted whenever a property created by this manager
	changes.

	\sa QtAbstractPropertyManager, QtAbstractPropertyBrowser, QtStringPropertyManager
*/

/*!
	\fn void QtDateTimeFormatPropertyManager::valueChanged(QtProperty *property, const QStringList &value)
...
/*!
	Creates a manager with the given \a parent.
*/
QtDateTimeFormatPropertyManager::QtDateTimeFormatPropertyManager(QObject *parent)
	: QtAbstractPropertyManager(parent), d_ptr(new QtDateTimeFormatPropertyManagerPrivate)
{
	d_ptr->q_ptr = this;
...
/*!
	Destroys this manager, and all the properties it has created.
*/
QtDateTimeFormatPropertyManager::~QtDateTimeFormatPropertyManager()
{
	clear();
}

/*!
	Returns the manager that produces the nested format string subproperties.
	...
	\sa QtAbstractPropertyBrowser::setFactoryForManager()
*/
QtStringPropertyManager *QtDateTimeFormatPropertyManager::subStringPropertyManager() const
{
...
/*!
	...
	\sa setValue()
*/
QStringList QtDateTimeFormatPropertyManager::value(const QtProperty *property) const
{
	return d_ptr->m_values.value(property, d_ptr->m_dateFormatsDefault);
}
...
QString QtDateTimeFormatPropertyManager::valueText(const QtProperty * /*property*/) const {
//	const QtDateTimeFormatPropertyManagerPrivate::PropertyValueMap::const_iterator it = d_ptr->m_values.constFind(property);
//	if (it == d_ptr->m_values.constEnd())
//		return QString();

	// For the parent show the generic text
	return tr("Date/time format strings");
}
...
*/
#
QIcon QtDateTimeFormatPropertyManager::valueIcon(const QtProperty * /*property*/) const
{
//	const QtDateTimeFormatPropertyManagerPrivate::PropertyValueMap::const_iterator it = d_ptr->m_values.constFind(property);
//	if (it == d_ptr->m_values.constEnd())
//		return QIcon();
...
/*!
	\fn void QtDateTimeFormatPropertyManager::setValue(QtProperty *property, const QStringList &value)
	...
	\sa value(), valueChanged()
*/
void QtDateTimeFormatPropertyManager::setValue(QtProperty *property, const QStringList &val)
{
	const QtDateTimeFormatPropertyManagerPrivate::PropertyValueMap::iterator it = d_ptr->m_values.find(property);
	if (it == d_ptr->m_values.end())
		return;
	...
	if(val.size() > 0)
		d_ptr->m_stringPropertyManager->setValue(d_ptr->m_propertyToMS[property], val[0]);
	else
		d_ptr->m_stringPropertyManager->setValue(d_ptr->m_propertyToMS[property], d_ptr->m_dateFormatsDefault[0]);

	if(val.size() > 1)
		d_ptr->m_stringPropertyManager->setValue(d_ptr->m_propertyToS[property], val[1]);
	else
		d_ptr->m_stringPropertyManager->setValue(d_ptr->m_propertyToS[property], d_ptr->m_dateFormatsDefault[1]);

	if(val.size() > 2)
		d_ptr->m_stringPropertyManager->setValue(d_ptr->m_propertyToMi[property], val[2]);
	else
		d_ptr->m_stringPropertyManager->setValue(d_ptr->m_propertyToMi[property], d_ptr->m_dateFormatsDefault[2]);

	if(val.size() > 3)
		d_ptr->m_stringPropertyManager->setValue(d_ptr->m_propertyToH[property], val[3]);
	else
		d_ptr->m_stringPropertyManager->setValue(d_ptr->m_propertyToH[property], d_ptr->m_dateFormatsDefault[3]);

	if(val.size() > 4)
		d_ptr->m_stringPropertyManager->setValue(d_ptr->m_propertyToD[property], val[4]);
	else
		d_ptr->m_stringPropertyManager->setValue(d_ptr->m_propertyToD[property], d_ptr->m_dateFormatsDefault[4]);

	if(val.size() > 5)
		d_ptr->m_stringPropertyManager->setValue(d_ptr->m_propertyToW[property], val[5]);
	else
		d_ptr->m_stringPropertyManager->setValue(d_ptr->m_propertyToW[property], d_ptr->m_dateFormatsDefault[5]);

	if(val.size() > 6)
		d_ptr->m_stringPropertyManager->setValue(d_ptr->m_propertyToMo[property], val[6]);
	else
		d_ptr->m_stringPropertyManager->setValue(d_ptr->m_propertyToMo[property], d_ptr->m_dateFormatsDefault[6]);

	if(val.size() > 7)
		d_ptr->m_stringPropertyManager->setValue(d_ptr->m_propertyToY[property], val[7]);
	else
		d_ptr->m_stringPropertyManager->setValue(d_ptr->m_propertyToY[property], d_ptr->m_dateFormatsDefault[7]);
...
/*!
	\reimp
*/
void QtDateTimeFormatPropertyManager::initializeProperty(QtProperty *property) {
	d_ptr->m_values[property] = d_ptr->m_dateFormatsDefault;

	QtProperty *propMS = d_ptr->m_stringPropertyManager->addProperty();
	propMS->setPropertyName(tr("Milliseconds"));
	d_ptr->m_stringPropertyManager->setValue(propMS, d_ptr->m_dateFormatsDefault[0]);
	d_ptr->m_propertyToMS[property] = propMS;
	d_ptr->m_MSToProperty[propMS] = property;
	property->addSubProperty(propMS);

	QtProperty *propS = d_ptr->m_stringPropertyManager->addProperty();
	propS->setPropertyName(tr("Seconds"));
	d_ptr->m_stringPropertyManager->setValue(propS, d_ptr->m_dateFormatsDefault[1]);
	d_ptr->m_propertyToS[property] = propS;
	d_ptr->m_SToProperty[propS] = property;
	property->addSubProperty(propS);

	QtProperty *propM = d_ptr->m_stringPropertyManager->addProperty();
	propM->setPropertyName(tr("Minutes"));
	d_ptr->m_stringPropertyManager->setValue(propM, d_ptr->m_dateFormatsDefault[2]);
	d_ptr->m_propertyToMi[property] = propM;
	d_ptr->m_MiToProperty[propM] = property;
	property->addSubProperty(propM);

	QtProperty *propH = d_ptr->m_stringPropertyManager->addProperty();
	propH->setPropertyName(tr("Hours"));
	d_ptr->m_stringPropertyManager->setValue(propH, d_ptr->m_dateFormatsDefault[3]);
	d_ptr->m_propertyToH[property] = propH;
	d_ptr->m_HToProperty[propH] = property;
	property->addSubProperty(propH);

	QtProperty *propD = d_ptr->m_stringPropertyManager->addProperty();
	propD->setPropertyName(tr("Days"));
	d_ptr->m_stringPropertyManager->setValue(propD, d_ptr->m_dateFormatsDefault[4]);
	d_ptr->m_propertyToD[property] = propD;
	d_ptr->m_DToProperty[propD] = property;
	property->addSubProperty(propD);

	QtProperty *propW = d_ptr->m_stringPropertyManager->addProperty();
	propW->setPropertyName(tr("Weeks"));
	d_ptr->m_stringPropertyManager->setValue(propW, d_ptr->m_dateFormatsDefault[5]);
	d_ptr->m_propertyToW[property] = propW;
	d_ptr->m_WToProperty[propW] = property;
	property->addSubProperty(propW);

	QtProperty *propMonth = d_ptr->m_stringPropertyManager->addProperty();
	propMonth->setPropertyName(tr("Months"));
	d_ptr->m_stringPropertyManager->setValue(propMonth, d_ptr->m_dateFormatsDefault[6]);
	d_ptr->m_propertyToMo[property] = propMonth;
	d_ptr->m_MoToProperty[propMonth] = property;
	property->addSubProperty(propMonth);

	QtProperty *propY = d_ptr->m_stringPropertyManager->addProperty();
	propY->setPropertyName(tr("Years"));
	d_ptr->m_stringPropertyManager->setValue(propY, d_ptr->m_dateFormatsDefault[7]);
	d_ptr->m_propertyToY[property] = propY;
	d_ptr->m_YToProperty[propY] = property;
	property->addSubProperty(propY);
...
/*!
	\reimp
*/
void QtDateTimeFormatPropertyManager::uninitializeProperty(QtProperty *property)
{
	QtProperty *propMS = d_ptr->m_propertyToMS[property];
	if (propMS) {
		d_ptr->m_MSToProperty.remove(propMS);
		delete propMS;
	}
	d_ptr->m_propertyToMS.remove(property);

	QtProperty *propS = d_ptr->m_propertyToS[property];
	if (propS) {
		d_ptr->m_SToProperty.remove(propS);
		delete propS;
	}
	d_ptr->m_propertyToS.remove(property);

	QtProperty *propMi = d_ptr->m_propertyToMi[property];
	if (propMi) {
		d_ptr->m_MiToProperty.remove(propMi);
		delete propMi;
	}
	d_ptr->m_propertyToMi.remove(property);

	QtProperty *propH = d_ptr->m_propertyToH[property];
	if (propH) {
		d_ptr->m_HToProperty.remove(propH);
		delete propH;
	}
	d_ptr->m_propertyToH.remove(property);

	QtProperty *propD = d_ptr->m_propertyToD[property];
	if (propD) {
		d_ptr->m_DToProperty.remove(propD);
		delete propD;
	}
	d_ptr->m_propertyToD.remove(property);

	QtProperty *propW = d_ptr->m_propertyToW[property];
	if (propW) {
		d_ptr->m_WToProperty.remove(propW);
		delete propW;
	}
	d_ptr->m_propertyToW.remove(property);

	QtProperty *propMo = d_ptr->m_propertyToMo[property];
	if (propMo) {
		d_ptr->m_MoToProperty.remove(propMo);
		delete propMo;
	}
	d_ptr->m_propertyToMo.remove(property);

	QtProperty *propY = d_ptr->m_propertyToY[property];
	if (propY) {
		d_ptr->m_YToProperty.remove(propY);
		delete propY;
	}
	d_ptr->m_propertyToY.remove(property);
