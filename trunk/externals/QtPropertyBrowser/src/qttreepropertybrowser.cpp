...
class QtTreePropertyBrowserScrollBar : public QScrollBar {
	Q_OBJECT
public:
	QtTreePropertyBrowserScrollBar(QWidget* p) : QScrollBar(p) {}
...
//	}
	...
	/*! Connected to collapsed/expand signals. */
...
private:
	/*! Function to recursivly sum up row heights. */
	int treeWidgetRowHeight(const QModelIndex & index ) const;
	...
	if (index.column() == 1 && m_editorPrivate) {
		...
		if (property && item && (item->flags() & Qt::ItemIsEnabled)) {
			...
			if (editor) {
				...
				// special handling in case of QCheckBox
				QtBoolEdit * box = qobject_cast<QtBoolEdit*>(editor);
				if (box != NULL) {
					box->setChecked(!box->isChecked()); // this will trigger the modification event
				}
			}
			return editor;
		}
...
QSize QtPropertyEditorDelegate::sizeHint(const QStyleOptionViewItem &option,
			const QModelIndex &index) const
{
	QSize s = QItemDelegate::sizeHint(option, index) + QSize(3, 4);
	QFontMetrics fm(option.font);
	int ls = fm.lineSpacing();
	int space = std::max<int>(ls*0.4, 6);
	if (ls+space > 20)
		s.setHeight(28);
	else
		s.setHeight(ls+space);
...
void QtTreePropertyBrowserPrivate::init(QWidget *parent)
{
	...
	QtTreePropertyBrowserScrollBar * scrollBar = new QtTreePropertyBrowserScrollBar(m_treeWidget);
	m_treeWidget->setHorizontalScrollBar(scrollBar);
	m_treeWidget->connect(scrollBar, &QtTreePropertyBrowserScrollBar::visibilityChanged,
			m_treeWidget, &QtPropertyEditorView::updateSizeConstraints);
...
void QtTreePropertyBrowser::setFixHeightToContents(bool enabled) {
	if (enabled) {

		connect( d_ptr->m_treeWidget, &QTreeWidget::itemCollapsed,
				 d_ptr->m_treeWidget, &QtPropertyEditorView::updateSizeConstraints );
		connect( d_ptr->m_treeWidget, &QTreeWidget::itemExpanded,
				 d_ptr->m_treeWidget, &QtPropertyEditorView::updateSizeConstraints );

		d_ptr->m_treeWidget->updateSizeConstraints();
	}
	else {
		disconnect( d_ptr->m_treeWidget, &QTreeWidget::itemCollapsed,
				 d_ptr->m_treeWidget, &QtPropertyEditorView::updateSizeConstraints );
		disconnect( d_ptr->m_treeWidget, &QTreeWidget::itemExpanded,
				 d_ptr->m_treeWidget, &QtPropertyEditorView::updateSizeConstraints );
	}
}
...
int QtPropertyEditorView::treeWidgetRowHeight(const QModelIndex & index ) const {
	int height = 0;
	...
	for( int r = 0; r < rowCount; ++r )
		height += treeWidgetRowHeight(model()->index(r,0,index) );
	return height;
}


void QtPropertyEditorView::updateSizeConstraints() {
	// we now sum up the heights of all visible rows
	int rowHeights = 0;

	int rowCount = topLevelItemCount();
	for (int i=0; i<rowCount; ++i)
		rowHeights += treeWidgetRowHeight(model()->index(i,0));

	rowHeights += header()->height(); // add header height
	rowHeights += 1; // add one for the top line
	if (horizontalScrollBar()->isVisibleTo(this))
		rowHeights += horizontalScrollBar()->height();

#ifdef Q_OS_WIN
	// set the constraints, add 1 so that scrollbars do not show up
	setMinimumHeight(rowHeights+4);
	setMaximumHeight(rowHeights+4);
#else
	setMinimumHeight(rowHeights+1);
	setMaximumHeight(rowHeights+1);
#endif
