...
void MasterSim::composeVariableVector() {
	...
	for (unsigned int i=0; i<m_project.m_graph.size(); ++i) {
		...
		std::size_t pos = edge.m_outputVariableRef.find("?");
		if (pos != std::string::npos) {

			// determine slave from filename
			IBK::Path filePath(edge.m_outputVariableRef.substr(0, pos));
			...
			std::string varName = edge.m_outputVariableRef.substr(pos+1);

			// check all slaves
			AbstractSlave * slave = NULL;
			for (unsigned int s=0; s<m_slaves.size(); ++s) {
				if (m_slaves[s]->m_filepath == filePath) {
					slave = m_slaves[s];
					break;
				}
			}
			if (slave == nullptr)
				throw IBK::Exception(IBK::FormatString("Unknown/undefined slave with filepath '%1'").arg(filePath), FUNC_ID);

			// now determine column index
			int colIndex = -1;
			try {
				colIndex = IBK::string2val<int>(varName);
				--colIndex; // data columns start with 1, but we have zero-based storage vectors
			}
			catch (...) {
				// not a column index, must be a variable name
			}
			if (colIndex >= (int)slave->m_doubleVarNames.size())
				throw IBK::Exception(IBK::FormatString("Invalid column index '%1' in file '%2'").arg(varName).arg(filePath), FUNC_ID);
			// search for a variable name
			if (colIndex == -1) {
				for (unsigned int i=0; i<slave->m_doubleVarNames.size(); ++i)
					if (varName == slave->m_doubleVarNames[i]) {
						colIndex = i;
						break;
					}
			}
