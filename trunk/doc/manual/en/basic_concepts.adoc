...
Once the communication interval is completed, the solver enters the time step adjustment loop. If time step adjustment is disabled via flag *adjustStepSize* (see <<_simulator_settings, Simulator settings>>), the loop content will only be executed once. For FMI 1.0 slaves, or FMI 2.0 slaves without capability for storing/restoring slave states, iteration is also not possible (actually, requesting an iterative algorithm for such slaves will trigger an error during initialzation).
