...
def validate_repo(vendor_dir, clean_up=False, platform_filter=None, fmi_type_filter=None):
    ...
    for subdir, dirs, files in os.walk(os.path.join(vendor_dir, 'results')):
        ...
        if fmi_type_filter != None and fmi_type != fmi_type_filter:
            continue
        
        if platform_filter != None and platform != platform_filter:
            continue
